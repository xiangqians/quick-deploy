package org.xiangqian.quick.deploy.comp;
...
public class ThreadPool {
    ...
    public ThreadPool() {
        ...
        // 线程空闲时的存活时间
        Duration keepAliveTime = Duration.ofMinutes(5);

        // 工作队列，用于存储被提交但尚未执行的任务
//        BlockingQueue<Runnable> workQueue = new ArrayBlockingQueue<>(64);
        ...
        // 拒绝策略，用于处理达到最大线程数且任务队列已满时的情况
        RejectedExecutionHandler handler = new ThreadPoolExecutor.AbortPolicy();

        this.executor = new ThreadPoolExecutor(corePoolSize, maximumPoolSize, keepAliveTime.getSeconds(), TimeUnit.SECONDS, workQueue, threadFactory, handler);
    }
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks, Duration duration) throws InterruptedException {
        return executor.invokeAll(tasks, duration.getSeconds(), TimeUnit.SECONDS);
    }
    ...
    public <T> T invokeAny(Collection<? extends Callable<T>> tasks, Duration duration) throws ExecutionException, InterruptedException, TimeoutException {
        return executor.invokeAny(tasks, duration.getSeconds(), TimeUnit.SECONDS);
    }
    ...
    public String toString() {
        ...
        builder.append("\n\t核心线程数：").append(executor.getCorePoolSize());
        builder.append("\n\t最大线程数：").append(executor.getMaximumPoolSize());
        builder.append("\n\t当前正在执行任务的线程数量：").append(executor.getActiveCount());
        builder.append("\n\t当前线程池中线程数量：").append(executor.getPoolSize());
        builder.append("\n\t任务队列中等待执行的任务数量：").append(executor.getQueue().size());
        builder.append("\n\t已经完成执行的任务数量：").append(executor.getCompletedTaskCount());
